...
class StrategyLearner(object):
    ...
    def computeDailyPortfolioValues(self,ordersDataFrame,dateRange,symbol,start_val = 1):
        pricesForDateRange = get_data([symbol], dateRange)
        ordersDataFrame = self.convertIndicesToDates(ordersDataFrame, pricesForDateRange)

        # Read in adjusted closing prices for given symbols, date range
        pricesForDateRange.loc[:,'Cash']=pd.Series(1,index=pricesForDateRange.index)

        #dfTrades is initialized with the prices dataframe, but will eventually be filled with trades.
        dfTrades = pricesForDateRange.copy()
        dfTrades.ix[:] = 0

        for index, row in ordersDataFrame.iterrows():
            if row['Order'] == "BUY":
                dfTrades.loc[pd.Timestamp.date(index), row['Symbol']] = float(row['Shares'])
                dfTrades.loc[pd.Timestamp.date(index), 'Cash'] =  float(row['Shares'] *-1 * float(pricesForDateRange.loc[pd.Timestamp.date(index),row['Symbol']]))

            elif row['Order']== "SELL":
                dfTrades.loc[pd.Timestamp.date(index), row['Symbol']] = -1 * float(row['Shares'])
                dfTrades.loc[pd.Timestamp.date(index), 'Cash'] =  float(row['Shares'])    * float(pricesForDateRange.loc[pd.Timestamp.date(index),row['Symbol']])
        ...
        print 'dfHoldings.shape1'
        print dfHoldings.shape[1]
        print 'dfholdings'
        print dfHoldings
        print 'dftrades'
        print dfTrades
        #Should set all initial holdings to whatever shares were bought on the first day
        for i in range (dfHoldings.shape[1]):
            dfHoldings.ix[0,i] = dfTrades.ix[0,i]

        print 'dfholdings before adding start val.  '
        print dfHoldings
        #Set cash at 0th row to start val minus whatever shares were bought on day 1
        dfHoldings.ix[0, dfHoldings.shape[1]-1] += start_val
        print 'dfholdings after adding start val and before cumsum'

        print dfHoldings
        #dfHoldings[:] = dfTrades.cumsum()

        shiftedTrades = dfTrades.copy().shift(1)

        print 'shapes'
        # print shiftedHoldings.shape
        print dfTrades.shape
        # print shiftedHoldings
        dfHoldings = shiftedTrades + dfTrades

        print 'dfholdings after cumsum'
        print dfHoldings

        #
        #Cash at all rows = cash + 1000000
        dfHoldings.ix[:,-1]= dfHoldings.ix[:,-1] + 1000000

        dfValues = dfHoldings * pricesForDateRange
    ...
    def showChart(self, portfolio,dates,symbol,startingPortfolioValue):
        ...
        # portvals = portvals * startingPortfolioValue
